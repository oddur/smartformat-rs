...
| Splitting | `plural`, `cond`, `choose`, `isnull`, `ismatch` and `list` cut their parts out of the parsed format on every render, which for a template rendered many times was over half the work. A `Format` therefore remembers the pieces it was cut into, keyed by separator and limit, in `SPLIT_CACHE_SLOTS` = 2 slots — two shapes because auto-detection hands one format to every auto-detecting formatter in turn and they do not split it alike: `list` stops at four separators where the other five split without a limit, so a single slot would leave every unnamed `{Count:one\|many}` cutting itself afresh forever. A third shape, which takes a formatter registered with a separator of its own, is cut and thrown away as every split was before the slots existed. A format holding no separator is *lent* rather than cloned: .NET splits it into itself, and all a formatter does with a one-piece split is count it and decline. The slots hang off a `OnceLock<Box<…>>` so that `Format` — embedded in every item of every format — stays the size it was, a clone starts empty, and they are outside `PartialEq` and `Debug`. That makes the tree read-only in practice — `items`, `raw`, `start` and `end` are accessors rather than public fields, so a `Format` is built with `Format::new` rather than as a struct literal — and it changes no answer: the pieces of a split are still cut eagerly as they always were and a piece that cannot be cut is still an `Err` of its own, so .NET's per-piece laziness is where it was |
